...
stop            = 1192;                     % final image #
step            = 1;                        % read images in steps
...
for i = start:step:stop                     % read images in the consequtive order
    ...
    while(1)
        ...
        elseif val == 122
            ...
            check1 = includes(test, 2)
            check2 = includes(test, 6)
            new_check = test.add(6);
            new_check.value
            ...
            for r = 1:2:row_num
                for c = 1:2:col_num
                    %if (~isequal(obs_Array(r:r+1,c:c+1), zeros(2, 2)))
                        obs_Array = connect_objects(r, c, obs_Array, conv_Array);
                    %end
                end
            end
